...
class DataRepository(private val dao: CourseDao) {
    ...
    fun getAllCourse(sortType: SortType): LiveData<PagedList<Course>> {
        val configuration = PagedList.Config.Builder()
            ...
            .build()
        return LivePagedListBuilder(dao.getAll(QueryUtil.sortedQuery(sortType)), configuration).build()
    }
    ...
    fun getTodaySchedule() : List<Course> {
        val todaySchedule = Calendar.getInstance().get(Calendar.DAY_OF_WEEK)
        return dao.getTodaySchedule(todaySchedule)
    }
